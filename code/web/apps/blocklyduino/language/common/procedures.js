...
Blockly.Language.procedures_defnoreturn = {
  ...
  mutationToDom: function() {
    ...
    for (var x = 0; x < this.arguments_.length; x++) {
      ...
      parameter.setAttribute('type', this.argument_types_[x]);
  ...
  domToMutation: function(xmlElement) {
    ...
    for (var x = 0, childNode; childNode = xmlElement.childNodes[x]; x++) {
      if (childNode.nodeName.toLowerCase() == 'arg') {
        ...
        this.argument_types_.push(childNode.getAttribute('type'));
      }
    }
    this.updateParams_();
  },
  ...
  compose: function(containerBlock) {
    ...
    while (paramBlock) {
      ...
      this.argument_types_.push(paramBlock.getTitleValue('TYPE'));
...
//  },
  declaredNames: function() {
  	 var names = [];
  	 for (var x = 0; x < this.arguments_.length; x++) {
  	 	names.push({ name: this.arguments_[x], type: this.argument_types_[x] });
  	 }
  	 return names;
  },
...
Blockly.Language.procedures_mutatorarg = {
  ...
  init: function() {
    ...
    this.appendDummyInput()
        ...
        .appendTitle(Blockly.LANG_PROCEDURES_MUTATORARGTYPE_TITLE)
